...
class PreDBClient:
    ...
    async def exact_release(self, release: str) -> dict | None:
        ...
        self.__class__._release_cache[key] = (time.monotonic() + ttl, match)
        return match
    ...
    async def nfo(self, pre_id: int) -> dict | None:
        ...
        self.__class__._nfo_cache[pre_id] = (
            time.monotonic() + self.NFO_CACHE_SECONDS,
            result,
        )
        return result
    ...
    async def teams(self) -> list[dict]:
        ...
        self.__class__._teams_cache = (
            time.monotonic() + self.TEAMS_CACHE_SECONDS,
            rows,
        )
        return rows
